...
def make_layer(in_channel,out_channel,num_block,stride):
    layers= []
    layers.append(ResidualBlock(in_channel,out_channel,stride,shortcut=True))
...
class ResNetTriplet(nn.Module):
    def __init__(self,embedding_size=256, num_classes=10):
        ...
        self.layer1 = make_layer(64, 128,  3, stride=1)
...
def ResNet34(embedding_size=256, num_classes=10):
    return ResNetTriplet(embedding_size, num_classes)
...
def run_facenet():
    ...
    model = ResNet34(embedding_size, 10)
